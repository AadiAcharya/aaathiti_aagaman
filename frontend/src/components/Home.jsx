  ...
  >
    ...
    <div className="p-5">
      <div className="flex items-center gap-2 mb-3">
        <span className="px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-full">{category}</span>
        <span className="text-text-muted text-xs">{date}</span>
      </div>
      <h3 className="text-text-primary font-bold text-base mb-2 group-hover:text-primary transition-colors line-clamp-2">{title}</h3>
      {excerpt && <p className="text-text-secondary text-sm line-clamp-2">{excerpt}</p>}
    </div>
  </div>
);
...
export default function Home() {
  ...
  const filtersActive = searchTerm || sortBy !== "default" || maxPrice !== 20000 || selectedTab !== "rooms";
  ...
  return (
    <div className="w-full bg-background min-h-screen">
      ...
      <div className="relative bg-bg-secondary overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-24">
          <div className="mb-10 text-center md:text-left">
            <p className="text-primary font-semibold text-sm tracking-widest uppercase mb-3">Welcome to</p>
            ...
            <p className="text-text-secondary text-lg md:text-xl max-w-xl mx-auto md:mx-0">
              Find your perfect stay — rooms, flats, villas & hostels across the country
            </p>
          </div>
          ...
          <div className="bg-background border border-primary/20 rounded-3xl p-6 md:p-8 shadow-2xl shadow-primary/5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
              <div>
                <label className="text-text-secondary text-xs font-semibold block mb-1 uppercase tracking-wide">Location</label>
                <input
                  value={searchTerm}
              ...
              <div>
                <label className="text-text-secondary text-xs font-semibold block mb-1 uppercase tracking-wide">Check In</label>
              ...
              <div>
                <label className="text-text-secondary text-xs font-semibold block mb-1 uppercase tracking-wide">Check Out</label>
                <input type="date" value={checkOut} min={checkIn} onChange={(e) => setCheckOut(e.target.value)}
                  className="w-full bg-bg-secondary text-text-primary rounded-xl py-3 px-4 focus:outline-none focus:ring-2 focus:ring-primary text-sm" />
              </div>
              <div>
                <label className="text-text-secondary text-xs font-semibold block mb-1 uppercase tracking-wide">Guests</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              <div>
                <label className="text-text-secondary text-xs font-semibold block mb-1 uppercase tracking-wide">Sort By</label>
                <select className="w-full bg-bg-secondary text-text-primary rounded-xl py-3 px-4 focus:outline-none focus:ring-2 focus:ring-primary text-sm"
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-text-secondary">
            ...
            <button onClick={resetFilters} className="bg-primary text-white px-6 py-3 rounded-full font-semibold transition hover:bg-primary-hover">
              Reset Filters
            </button>
          </div>
        ) : (
        ...
        <div className="bg-bg-secondary/50 py-16 md:py-20">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <SectionHeader
              ...
              action={
                <button onClick={() => navigate("/properties")} className="text-primary font-semibold hover:underline text-sm whitespace-nowrap">
                  View All →
                </button>
              }
            />
      ...
      {featured.length > 0 && (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20">
          <SectionHeader title="Featured Properties" subtitle="Handpicked properties for an exceptional stay" />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featured.map((property, i) => (
              <div key={property._id || property.id} className={i === 0 ? "md:row-span-2" : ""}>
                <div onClick={() => navigate("/properties")}
                  ...
                  {property.image ? (
                    ...
                    <div className="w-full h-full bg-primary/10 flex items-center justify-center" style={{ minHeight: i === 0 ? "400px" : "200px" }}>
                      <span className="text-6xl">🏠</span>
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <p className="text-white/70 text-xs font-semibold uppercase tracking-wide mb-1">Featured</p>
                    <h3 className="text-white font-bold text-lg mb-1">{property.title}</h3>
                    <p className="text-white/80 text-sm">📍 {property.location}</p>
                    <p className="text-accent font-bold mt-2">{property.price}</p>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); toggleFavorite(property._id || property.id); }}
                    ...
                    {favorites.includes(property._id || property.id) ? "❤️" : "🤍"}
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="bg-bg-secondary/50 py-16">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <SectionHeader
              ...
              subtitle={favoriteProperties.length === 0 ? "Scroll down to add properties to your favorites" : undefined}
            />
            {favoriteProperties.length === 0 ? (
              <div className="text-center py-8 text-text-secondary">
                <p className="text-4xl mb-3">💔</p>
                <p>Your saved favorites are from a previous session and are no longer available.</p>
                <button onClick={() => { setFavorites([]); localStorage.removeItem("favorites"); }}
                  className="mt-4 text-primary underline text-sm">Clear old favorites</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20">
        <div className="bg-gradient-to-br from-primary/10 to-accent/5 rounded-3xl p-8 md:p-12 border border-primary/20 overflow-hidden relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center relative">
            <div>
              <p className="text-primary font-semibold text-sm tracking-widest uppercase mb-3">For Property Owners</p>
              <h2 className="text-3xl md:text-4xl font-bold text-text-primary mb-4">
                Try Hosting<br />With Us
              </h2>
              <p className="text-text-secondary mb-6 leading-relaxed">
                Earn extra income by listing your property. Join thousands of hosts earning with Aatithi Aagaman. Full control over your listings, pricing, and availability.
              </p>
              <div className="grid grid-cols-2 gap-3 mb-8 text-sm text-text-secondary">
                {["Set your own price", "Full calendar control", "Instant payouts", "24/7 host support"].map((f) => (
                  <div key={f} className="flex items-center gap-2"><span className="text-primary">✓</span> {f}</div>
                ))}
              </div>
            ...
            <div className="h-64 md:h-80 bg-background rounded-2xl overflow-hidden border border-primary/10">
              <img src="/images/hosting.svg" alt="Start Hosting" className="w-full h-full object-cover" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-bg-secondary/50 py-16 md:py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <SectionHeader
            ...
            action={
              <button onClick={() => navigate("/rental-guide")}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {blogs.slice(0, 3).map((blog) => (
              <BlogCard key={blog.id} {...blog} onClick={() => navigate("/rental-guide")} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          <div className="h-72 bg-bg-secondary rounded-2xl overflow-hidden border border-text-muted/10 order-2 md:order-1">
            <img src="/images/browse.svg" alt="Browse Properties" className="w-full h-full object-cover" />
          </div>
          <div className="order-1 md:order-2">
            ...
            <p className="text-text-secondary mb-8 leading-relaxed">
              Explore our full catalogue of properties sorted by type, location, and price range.
            </p>
      ...
      <div className="bg-bg-secondary/50 py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="bg-gradient-to-r from-primary/10 to-accent/5 rounded-3xl p-8 md:p-12 border border-primary/10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
              <div>
                <p className="text-primary font-semibold text-sm tracking-widest uppercase mb-3">Mobile App</p>
                <h2 className="text-3xl md:text-4xl font-bold text-text-primary mb-4">Book On The Go</h2>
                <p className="text-text-secondary mb-8 leading-relaxed">
                  Download our mobile app and manage your bookings, get notifications, and explore properties anywhere, anytime.
                </p>
                <div className="flex flex-wrap gap-3">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-text-secondary mb-6 leading-relaxed">
              Aatithi Aagaman connects guests with exceptional properties across the country. Whether you're looking for a cozy room, a modern flat, a luxury villa, or a budget hostel — we have it all.
            </p>
            <p className="text-text-secondary mb-8 leading-relaxed">
              Our platform makes finding and booking your next stay simple, safe, and seamless — with verified listings, real reviews, and secure payments.
            </p>
            <div className="space-y-2 mb-8">
              {[
                ...
                <p key={label} onClick={() => navigate(path)}
          ...
          <div className="h-72 md:h-96 bg-bg-secondary rounded-2xl overflow-hidden border border-text-muted/10">
            <img src="/images/about.svg" alt="About Us" className="w-full h-full object-cover" />
          </div>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
            <div className="text-center md:text-left shrink-0">
              <h3 className="text-2xl font-bold text-text-primary mb-1">Stay In The Loop</h3>
              <p className="text-text-secondary text-sm">Get the latest listings and rental tips in your inbox</p>
            </div>
